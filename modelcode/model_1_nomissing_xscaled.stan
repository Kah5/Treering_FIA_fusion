...
transformed parameters{
   ...
   matrix[Nrow, Ncol+1] xscaled;//make xscaled 1 column longer so we can use t-1 for the diameter effect in the liklihood function
...
model {
  ...
  xinit  ~ uniform(0, 75);//uninformative prior on xinit

  
 // tree ring diameter increment process model liklihood
 for(i in 1:Nrow){//loop over the number of trees
     inc[i,1:Ncol] ~ lognormal( alpha_TREE[i] + betaX*xscaled[i, 1:(Ncol)], sigma_add);
 }
