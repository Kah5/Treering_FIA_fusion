...
//data & process model for increment
  for(i in 1:Nrow){
    ...
    xinit[i]  ~ uniform(0, 75);
     ...
     for(t in 2:Ncol){
       inc[i,t] ~ lognormal(alpha_TREE[i] + beta_YEAR[t] + 
       betaMAP*MAP[i] + betaMAT*MAT[i] + betaX*x[i,t-1] + 
       betaTmax*tmaxAprMayJunscaled[i,t]+ 
       betaPrecip*wateryrscaled[i,t] + betaSDI*SDI[i,t], sigma_add);// + betaPrecip_MAT*wateryrscaled[i,t]*MAT[i] + betaSDI*SDI[i,t] + betaPrecip_Tmax*wateryrscaled[i,t]*tmaxAprMayJunscaled[i,t], sigma_add);
