...
model {
 ...
 for(i in 1:Nrow){//loop over the number of trees
        ...
        inc[i,1:Ncol] ~ lognormal( alpha_TREE[i] + betaX*xscaled[i, 1:(Ncol)] + 
        betaTmax*tmaxAprMayJunscaled[i,1:Ncol] + betaPrecip*wateryrscaled[i,1:Ncol] +
        betaSDI*SDI[i,1:Ncol] + betaMAT*MAT[i] + betaMAP*MAP[i], sigma_add); 
