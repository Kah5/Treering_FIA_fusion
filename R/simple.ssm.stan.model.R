...
model.name <- "small_ssm_stan_sigma_dbh_normal_dbh_normal_full_X_interactions_sigma_constrained"

fit.betaX.tmax.precip <- stan(model_code = mod.ssm.stan , 
                              data = mod.data,
            iter = 3000, 
            chains = 3, 
            verbose=FALSE, 
...
#posterior <- as.array(fit.betaX.tmax)
#dim(posterior)
#dimnames(posterior)
csvfiles <-  paste0("full_ssm_stan_sigma_dbh_normal_dbh_normal_full_X_interactions_sigma_constrained_", 1:3, ".csv")

fit.betaX.tmax.precip1 <- rstan::read_stan_csv(csvfiles[1])
fit.betaX.tmax.precip2 <- rstan::read_stan_csv(csvfiles[2])
fit.betaX.tmax.precip3 <- rstan::read_stan_csv(csvfiles[3])
...
x.pred <- dplyr::select(as.data.frame(fit.betaX.tmax.precip),"x[1,1]":"x[100,36]")
inc.pred <- dplyr::select(as.data.frame(fit.betaX.tmax.precip),"inc[1,1]":"inc[100,36]")
