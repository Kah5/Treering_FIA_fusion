...
model.name <- "RE.MAP.MAT.X.timevaryingclimate.SDI"


# fit a bunch of models:

# null model:
fit.3 <- stan(file = 'model_3.stan' , 
              data = mod.data,
              iter = 3000, 
              chains = 3, 
              verbose=FALSE, 
              ...
              pars = c("mu", "sigma_inc", "sigma_add", "sigma_dbh","beta_YEAR", "alpha_TREE", 
                       "betaMAP", "betaMAT","betaX","betaTmax", "betaPrecip","betaSDI",
                       "x", "inc")) # , init = initfun)
...
#par(mfrow = c(5, 3))
traceplot (fit.3, pars = par.names, nrow = 8, ncol = 3, inc_warmup = FALSE) 
dev.off()
...
x.pred <- dplyr::select(as.data.frame(fit.3),"x[1,1]":"x[100,36]")
inc.pred <- dplyr::select(as.data.frame(fit.3),"inc[1,1]":"inc[100,36]")
...
save(ll, r_eff, leaveoneout, file = here::here("looresults", "model_3_loo_small.RData"))
