...
#   load(paste0(workingdir,"/IGF_PIPO_region/", file.base.name,i,".RData"))
#   new.out <- jags.out 
#   
...
# saveRDS(jags.comb, paste0("data/output/IGF_Xvals_", output.base.name, ".RDS"))

#load(file="output/random_effects_model_simple.1.RData")
jags.comb = model.out

#data <- readRDS("jags.data.formatted.rds")
#init <- readRDS("jags.init.formatted.rds")
...
# cov.data.ordered <- cov.data.regional
...
pdf(paste0("output/plot_held_out_dbh",model.name,".pdf"))
...
for (i in smp) {
  ...
  points(data$time, data$z[i, 1:36], pch = "+", cex = 1.5)
...
# saveRDS(out.sample.dbh.df, paste0("data/output/",output.base.name,"pred.obs.out.of.sample.dbh.rds"))
...
saveRDS(in.sample.dbh.df, paste0("output/",model.name,"pred.obs.within.sample.dbh.rds"))
...
p.o.within.sample
ggsave( device = "png", height = 5, width = 10, units = "in", paste0("output/",model.name, "_DBH_held_out_p.o.plots.png"))
...
#above code commented out because we dont have out of sample yet
#--------------------------------------------------------------------------
# Validation for Increment (within sample only)
#--------------------------------------------------------------------------
pdf(paste0("output/rw_insample_increment",model.name,".pdf"))
layout(matrix(1:8, 4, 2, byrow = TRUE))
out      <- as.matrix(model.out) ### LOADS MCMC OUTPUT INTO OBJECT "OUT"
...
for (i in smp) {
  ...
  in.sample.inc[[i]] <- data.frame(inc.data = data$y[i,],
                                   year = data$time,
...
saveRDS(in.sample.inc.df, paste0("output/",model.name,"pred.obs.out.of.sample.inc.rds"))
...
ggsave(device = "png", height = 5, width = 5, units = "in",  paste0("output/", model.name, "_increment_in_sample_p.o.plots.png"))
...
# In sample residuals plots:#
#################################
# cov.data$id <- 1:length(cov.data$CORE_CN)
# 
