...
model.name <- "RE.MAP.MAT.X.timevaryingclimate.SDI.all.interactions.X2"


# fit a bunch of models:

# null model:
fit.7 <- stan(file = 'model_7.stan' , 
              data = mod.data,
              iter = 3000, 
              chains = 3, 
              verbose=FALSE, 
              ...
              pars = c("mu", "sigma_inc", "sigma_add", "sigma_dbh","beta_YEAR", "alpha_TREE", 
                       "betaMAP", "betaMAT","betaX","betaX2","betaTmax", "betaPrecip","betaSDI",
...
saveRDS(fit.7, here(paste0("small_model_fits/", model.name, ".RDS")))
fit.7 <- readRDS(here(paste0("small_model_fits/", model.name, ".RDS")))

# Warning messages:
#   1: There were 575 divergent transitions after warmup. See
# https://mc-stan.org/misc/warnings.html#divergent-transitions-after-warmup
# to find out why this is a problem and how to eliminate them. 
# 2: Examine the pairs() plot to diagnose sampling problems
# 
# 3: The largest R-hat is 1.41, indicating chains have not mixed.
...
#par(mfrow = c(5, 3))
traceplot (fit.7, pars = par.names, nrow = 8, ncol = 4, inc_warmup = FALSE) 
dev.off()

pairs(fit.7, pars = c("mu", "sigma_inc", 
                      "sigma_add", 
                      "sigma_dbh", 
                      "betaMAP", "betaMAT", "betaTmax", "betaPrecip","betaSDI"))


x.pred <- dplyr::select(as.data.frame(fit.7),"x[1,1]":"x[100,36]")
inc.pred <- dplyr::select(as.data.frame(fit.7),"inc[1,1]":"inc[100,36]")
...
fit_ssm_df <- as.data.frame(fit.7) # takes awhile to convert to df
...
#fit_ssm_df <- as.data.frame(fit.7) # takes awhile to convert to df
covariates = c("betaX","betaX2", "betaMAP","betaMAT",  "betaTmax", "betaPrecip","betaSDI", 
               "betaPrecip_MAP","betaPrecip_MAT","betaPrecip_Tmax",  "betaPrecip_SDI",
               "betaTmax_MAP", "betaTmax_MAT","betaTmax_SDI",
               "betaX_Precip", "betaX_Tmax", "betaX_SDI", "betaMAP_MAT",
               "betaX_MAP", "betaX_MAT")
...
# plot the size effects here:
size_effects <- cov.estimates %>% select("betaX", "betaX2")

dia <- 10:80
effect <- matrix(NA, nrow = nrow(size_effects), ncol = length(dia))
for(i in 1:length(dia)){
  effect[,i]<- (size_effects$betaX*dia[i]) + size_effects$betaX2*(dia[i]^2)
}
...
ggsave(here("model_simple_run/output", paste0("size_effect_", model.name, ".png")))
...
for(i in 1:mod.data$Nrow){
  ...
  for(t in 1:mod.data$Ncol){
    increment_mu[i,,t] <- as.matrix(alpha_trees[, i]) + as.matrix(beta_years[,t]) + 
      ...
      cov.estimates$betaX*x.pred[,paste0("x[",i,",", t,"]")] + 
      cov.estimates$betaX2*x.pred[,paste0("x[",i,",", t,"]")]^2+ 
...
save(ll, r_eff, leaveoneout, file = here::here("looresults", "model_7_loo_small.RData"))
