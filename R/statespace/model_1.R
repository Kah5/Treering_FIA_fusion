...
model.name <- "RE.MAP.MAT.X"


# fit a bunch of models:

# null model:
fit.1 <- stan(file = 'model_1.stan' , 
              data = mod.data,
              iter = 3000, 
              chains = 3, 
              verbose=FALSE, 
              ...
              pars = c("mu", "sigma_inc", "sigma_add", "sigma_dbh","beta_YEAR", "alpha_TREE", 
                       "betaMAP", "betaMAT","betaX",
                       "x", "inc")) # , init = initfun)

saveRDS(fit.1, here(paste0("small_model_fits/", model.name, ".RDS")))

fit.1 <- readRDS( here(paste0("small_model_fits/", model.name, ".RDS")))

posterior <- as.array(fit.1 )
...
#par(mfrow = c(5, 3))
traceplot (fit.1, pars = par.names, nrow = 8, ncol = 3, inc_warmup = FALSE) 
dev.off()
...
x.pred <- dplyr::select(as.data.frame(fit.1),"x[1,1]":"x[100,36]")
inc.pred <- dplyr::select(as.data.frame(fit.1),"inc[1,1]":"inc[100,36]")
...
for(i in 1:mod.data$Nrow){
  ...
  for(t in 1:mod.data$Ncol){
    increment_mu[i,,t] <- as.matrix(alpha_trees[, i]) + as.matrix(beta_years[,t]) + cov.estimates$betaMAP*data$MAP[i] + cov.estimates$betaMAT*data$MAT[i] + cov.estimates$betaX*x.pred[,paste0("x[",i,",", t,"]")]
...
save(ll, r_eff, leaveoneout, file = here::here("looresults", "model_1_loo_small.RData"))
...
traceplot(fit.1, pars = "beta_YEAR[1]", inc_warmup = FALSE)
traceplot(fit.1, pars = "beta_YEAR[15]", inc_warmup = FALSE)
