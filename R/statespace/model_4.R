...
model.name <- "RE.MAP.MAT.X.timevaryingclimate.SDI.most.interactions"


# fit a bunch of models:

# null model:
fit.4 <- stan(file = 'model_4.stan' , 
              data = mod.data,
              iter = 3000, 
              chains = 3, 
              verbose=FALSE, 
...
#par(mfrow = c(5, 3))
traceplot (fit.4, pars = par.names, nrow = 8, ncol = 4, inc_warmup = FALSE) 
dev.off()
...
inc.pred <- dplyr::select(as.data.frame(fit.4),"inc[1,1]":"inc[100,36]")
...
save(ll, r_eff, leaveoneout, file = here::here("looresults", "model_4_loo_small.RData"))
