...
dir.path = "/Users/kellyheilman/Documents/Treering_FIA_fusion_PIPO/data/" # where the distribution shape files are
...
if(file.exists(paste0(dir.path, spp, "/"))){
  spp.distribution <- st_read(paste0(dir.path, spp, "/"))
  #plt.shp <- spp.distribution
  #plt.spp <- plt.shp %>% filter(shp.code %in% spp)
  
  # plot distribution 
 US.spp.map <-  spp.distribution %>% 
    ggplot() +
