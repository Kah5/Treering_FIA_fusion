...
rm(POP_STRATUM, POP_EVAL_TYP, POP_ESTN_UNIT)
...
scale.mort.expns <- function( parse = "full", scale.mort.prob = 1) {
  mort.df <- readRDS( paste0("outputs/", "all.plot.mort.C.60SDIthresh_", scale.mort.prob, ".RDS"))
    ...
    plot.periodic <- unique(periodic.data[, c("EVAL_TYP","ECOSUBCD", "ESTN_UNIT_CN", "ESTN_METHOD", "STRATUM_CN", "PLT_CN", "EXPNS")])
    AGB.dead.distinct <- AGB.dead %>% distinct()
...
full.1 <- scale.mort.expns( parse = "full", scale.mort.prob = 1)
full.1.1 <- scale.mort.expns( parse = "full", scale.mort.prob = 1.1)
full.0.9 <- scale.mort.expns( parse = "full", scale.mort.prob = 0.9)
...
scale.all.live.expns <- function( parse = "full", scale.mort.prob = 1) {
  AGB <- readRDS(paste0("outputs/parse.DIDD.mort.60SDIthreshold_",scale.mort.prob,".RDS"))
  ...
  AGB.live.distinct <- AGB.live %>% distinct()
  AGB.live.EXPNS <- left_join(AGB.live.distinct, plot.periodic %>%  filter(EVAL_TYP == 'EXPVOL'))
  AGB.test.EXPNS <- AGB.live.EXPNS %>% filter(rcp %in% "rcp26" & parse %in% "full" & year %in% 2002)
  AGB.live.EXPNS %>% filter(PLT_CN %in% unique(AGB.live.EXPNS$PLT_CN)[1] & rcp %in% "rcp85" & parse %in% "full") %>% 
    mutate(exp.mAGB_kg = mAGB*EXPNS) %>% 
    mutate(exp.mAGB.Tg = (exp.mAGB_kg*0.501) /1000000000 )
...
full.live.1 <- scale.all.live.expns( parse = "full", scale.mort.prob = 1)
full.live.1.1 <- scale.all.live.expns( parse = "full", scale.mort.prob = 1.1)
full.live.0.9 <- scale.all.live.expns( parse = "full", scale.mort.prob = 0.9)
...
hist(full.live.1$mAGB)
hist(full.live.1.1$mAGB)
hist(full.live.0.9$mAGB)

ending.full.live <- rbind(full.live.1, full.live.0.9, full.live.1.1)

# do the same for the dead trees
ending.mort.1 <- full.1 #%>% filter(parse %in% "full" & year %in% 2098)
ending.mort.1.1 <- full.1.1 #%>% filter(parse %in% "full" & year %in% 2098)
ending.mort.0.9 <- full.0.9 #%>% filter(parse %in% "full" & year %in% 2098)
...
parse.nm <- "GD.10"
for( parse.nm in c("full", "DD.ramp", "GD.10", "GD.20", "no climate change")){
  
  
full.low.high.live <- ending.full.live %>% filter(parse %in% parse.nm & year %in% 2098) %>% 
  select(rcp, parse, mort.scheme, year,scaled.mortality, lowA, upA, mAGB)
#full.low.high.m <- reshape2::melt(full.low.high, id.vars = c("rcp","parse", "mort.scheme", "scaled.mortality","year"))
full.low.high.dead <- ending.mort %>% mutate(parse = ifelse(parse %in% "noCC", "no climate change", parse))%>% filter(parse %in% parse.nm & year %in% 2098) %>% 
  select(rcp, parse, mort.scheme, year,scaled.mortality, lowAGB.dead, hiAGB.dead, mAGB.dead)

## do the math here

# join the dead and live: 
full.low.high.dead.live <- left_join(full.low.high.live, full.low.high.dead)
...
sens.C.stoage.mort
ggsave(height = 4, width = 8, units = "in", paste0("outputs/SENS_C_STORAGE_2098_",parse.nm,".png"))
...
low.high.Cpools$`Carbon Pool` <- factor(low.high.Cpools$`Carbon Pool`, levels = c( "Live Tree C", "Standing Dead C"))
totalC.barplot <- ggplot(data = low.high.Cpools, aes(x = interaction(rcp, scaled.mortality), y = mAGB, group = rcp, fill = `Carbon Pool`), alpha = 0.1)+
  geom_bar(stat = "identity", position = position_stack(reverse = TRUE), width = 0.7)+#facet_wrap(~`Carbon Pool`)+
...
ggsave(height = 4, width = 6.5, units = "in", paste0("outputs/mean_total_C_live_dead_mort_sens_",parse.nm,".png"))

library(cowplot)
png(height = 8, width = 10, units = "in", res = 200, paste0("outputs/combined_mortality_figures_",parse.nm,".png"))
...
png(height = 8, width = 10, units = "in", res = 200, paste0("outputs/combined_mortality_figures_",parse.nm,".png"))
cowplot::plot_grid(mort.CC.pct.bar + theme(legend.position= "none")+ylab("% Mortality atttributed \n to climate change"), totalC.barplot, align = "h", ncol = 2, rel_widths = c(0.9, 1.5), 
                             labels = c("A", "B"))
...
# compare to regional carbon at the beginning of the time series
scale.obs.expns <- function( parse = "full", scale.mort.prob = 1) {
  mort.df <- readRDS( paste0("outputs/", "all.plot.mort.C.60SDIthresh_", scale.mort.prob, ".RDS"))
  ...
  plot.periodic <- unique(periodic.data[, c("ECOSUBCD","EVAL_TYP", "ESTN_UNIT_CN", "ESTN_METHOD", "STRATUM_CN", "PLT_CN", "EXPNS", "CARBON_AG", "YEAR")])
  # AGB.dead.distinct <- AGB.dead %>% distinct()
  # AGB.dead.EXPNS <- left_join(AGB.dead, plot.periodic)
  # 
  
  # ## Estimate Tree totals
  # tre_bioGrp <- periodic.data %>%
  #   filter(EVAL_TYP == 'EXPVOL') %>%
  #   ## Make sure we only have unique observations of plots, trees, etc.
  #   distinct(ECOSUBCD, ESTN_UNIT_CN, STRATUM_CN, PLT_CN, CONDID, SUBP, TREE,year, .keep_all = TRUE) %>%
  #   ## Plot-level estimates first (multiplying by EXPNS here)
  ...
  # get general summary of the total mortality in terms of C for each mortality type
 observed <- plot.periodic %>%
    #dplyr::mutate(across(mAGB.dead:hiAGB.dead.di, function(x){x*EXPNS})) %>%
    #group_by(PLT_CN, mort.scheme, rcp, year, parse) %>%
    group_by(YEAR, ECOSUBCD, ESTN_UNIT_CN, ESTN_METHOD, STRATUM_CN, PLT_CN) %>%
    mutate(across(CARBON_AG, function(x){x*EXPNS})) %>%
    # mutate(bioPlot = mAGB*EXPNS , # out mAGB is in kg (?), assuming acre plto
    #           carbPlot = bioPlot*0.501) %>%  ## Now we simply sum the values of each plot (expanded w/ EXPNS)
    ## to obtain population totals
    summarise(across(CARBON_AG, function(x){((x)/0.000453592)/0.404686})) %>% 
    ungroup() %>% # sum across all the PLT_CNs
    group_by(YEAR) %>%
    summarise(across(c(CARBON_AG), sum))
...
tre_bioGrp <- periodic.data %>%
  ...
  ## Plot-level estimates first (multiplying by EXPNS here)
  group_by(YEAR, ECOSUBCD, ESTN_UNIT_CN, ESTN_METHOD, STRATUM_CN, PLT_CN) %>%
  summarize(bioPlot = DRYBIO_AG*EXPNS , # out mAGB is in kg (?), assuming acre plto
            carbPlot = CARBON_AG*EXPNS) %>%  ## Now we simply sum the values of each plot (expanded w/ EXPNS)
  ## to obtain population totals
  group_by(YEAR, ECOSUBCD) %>%
  summarize(BIO_AG_TOTAL = sum(bioPlot, na.rm = TRUE),
            CARB_AG_TOTAL = sum(carbPlot, na.rm = TRUE))
...
tre_area_bioGrp <- left_join(tre_bioGrp,area_bioGrp)
test.summary <- tre_area_bioGrp %>% mutate(CARB_AG_per_AREA_lb_acre = CARB_AG_TOTAL/AREA_TOTAL) %>% 
  mutate(CARB_AG_per_AREA_Mg_ha = (CARB_AG_per_AREA_lb_acre*0.000453592)/0.404686) %>% 
  summarise(total.C = sum(CARB_AG_per_AREA_Mg_ha))
rm( data, db, POP_PLOT_STRATUM_ASSGN, TREE, COND, PLOT)
rm(POP_STRATUM, POP_EVAL_TYP, POP_ESTN_UNIT)
# periodic.data <- periodic.data %>%
#   mutate(YEAR = END_INVYR) %>%
