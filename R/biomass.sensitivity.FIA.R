biomass.sensitivity.FIA <- function(plot, density.dependent = TRUE, density.independent = TRUE, scenario = "rcp26", SDI.ratio.DD = 0.8, aggressiveCC = FALSE){
  ...
  if(nrow(trees.in.plt)<=1){
    ...
    plot2AGB_noSDI(combined = combined, out = out.mean, tpa = tpa.mean, tpa.diff = tpa.diff.mean, mort.scheme = mort.scheme, allom.stats = kaye_pipo, unit.conv = 0, plot = plot, yrvec = 2001:2018, scenario = scenario,cc.scenario = cc.scenario, p = p, p.inc = p.inc, SDI.ratio.DD = SDI.ratio.DD)
