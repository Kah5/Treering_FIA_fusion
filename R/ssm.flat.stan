...
//data & process model for increment
  for(i in 1:Nrow){
    ...
    xinit[i]  ~ uniform(0, 75);
     ...
     for(t in 2:Ncol){
       inc[i,t] ~ lognormal(alpha_TREE[i]+ beta_YEAR[t] + betaX*x[i,t-1]+ betaTmax*tmaxAprMayJunscaled[i,t]+ betaPrecip*wateryrscaled[i,t]+ betaMAP*MAP[i] + betaMAT*MAT[i] + betaPrecip_MAT*wateryrscaled[i,t]*MAT[i] + betaSDI*SDI[i,t] + betaPrecip_Tmax*wateryrscaled[i,t]*tmaxAprMayJunscaled[i,t] + betaPrecip_SDI*wateryrscaled[i,t]*SDI[i,t] + betaTmax_SDI*tmaxAprMayJunscaled[i,t]*SDI[i,t] + betaTmax_MAP*tmaxAprMayJunscaled[i,t]*MAP[i] + betaTmax_MAT*tmaxAprMayJunscaled[i,t]*MAT[i]+ betaX_Tmax*x[i,t-1]*tmaxAprMayJunscaled[i,t]+ betaX_Precip*x[i,t-1]*wateryrscaled[i,t]+ betaX_SDI*x[i,t-1]*SDI[i,t], sigma_add);# + betaMAP*MAP[i] + betaMAT*MAT[i] + betaPrecip_MAT*wateryrscaled[i,t]*MAT[i] + betaSDI*SDI[i,t] + betaPrecip_Tmax*wateryrscaled[i,t]*tmaxAprMayJunscaled[i,t], sigma_add);
       //inc[i,t] ~ lognormal(alpha_TREE[i]+ beta_YEAR[t] , sigma_add);
       //+ betaMAP*MAP[i] + betaMAT*MAT[i] + betaPrecip_MAT*wateryrscaled[i,t]*MAT[i] + betaSDI*SDI[i,t] + betaPrecip_Tmax*wateryrscaled[i,t]*tmaxAprMayJunscaled[i,t], sigma_add);
       //inc[i,t] ~ lognormal(alpha_TREE[i]+ beta_YEAR[t] , sigma_add);
